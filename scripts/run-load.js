...
async function spawnRun(runId, initiator, numSessions, numOrders, restockInterval) {
  ...
  // 4) Sessions in parallel with a small stagger
  await Promise.all(
    Array.from({length:numSessions}).map(async(_,idx)=>{
      ...
      const user = await sessionRetry(
        ()=>client.user.upsert({where:{email:username},update:{},create:{email:username}}),
        'upsert-user'
      );
      const cart = await sessionRetry(
        ()=>client.cart.upsert({where:{userId:user.id},update:{},create:{userId:user.id}}),
        'upsert-cart'
      );
      ...
      for (let i=0; i<numOrders; i++) {
        ...
        await sessionRetry(async()=>{
          await Promise.all(picked.map(({id:itId,stock})=>{
        ...
        const newOrder = await sessionRetry(
          ()=>client.order.create({
            data:{
              ...
              items:{create:picked.map(({id,price,stock,description,src})=>({
                itemId:id,quantity:1,priceCents:Math.round(price*100),
                description,src
              }))}
            }
          }),
        ...
        // C) Per-item adjust in micro-transactions
        for (const {id:itemId} of picked) {
          // pre-read outside tx
          const inv = await client.inventory.findUnique({
            where:{itemId},select:{id:true,onHand:true,reserved:true}
          });
          if (!inv) continue;
      ...
      await sessionRetry(
        ()=>client.loadRunSummary.create({
          data:{runId,username,ordersCompleted:sessionOrders,
                startTime:sessionStart,endTime:new Date()}
        }),
        'write-summary'
      );
    })
  ...
  // 6) Cleanup leftover reservations + status promotion + finalize...
  console.log('› Cleaning up stray reservations…');
  await client.cartItem.deleteMany({});
  await client.$executeRawUnsafe(`
    UPDATE "Inventory"
    SET "onHand" = "onHand" + "reserved", "reserved" = 0
    WHERE "reserved" > 0
  `);

  // (promotion pass as before…)
